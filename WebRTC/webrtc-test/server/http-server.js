...
// Create HTTP server (NOT SECURE - FOR TESTING ONLY)
const server = http.createServer(app);

// Socket.io with HTTP support
...
function handleUserLeaving(socket) {
  ...
  if (user && user.roomId) {
    ...
    if (room) {
      ...
      console.log(`👋 User ${user.userId} left room ${user.roomId}`);
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`🚀 HTTP Signaling server running on http://0.0.0.0:${PORT}`);
