/**
 * Room.js - Room Creation and Joining Component (Commented Version)
 * 
 * This component provides the user interface for:
 * - Creating new video call rooms
 * - Joining existing rooms
 * - Copying room IDs to clipboard
 * - Displaying instructions for using the app
 */
 ...
 * Room Component
 * 
 * @param {function} onCreateRoom - Callback function to create a new room
 * @param {function} onJoinRoom - Callback function to join an existing room
 * @param {string} error - Error message to display (if any)
 */
const Room = ({ onCreateRoom, onJoinRoom, error }) => {
  // State for room ID input field
  const [roomId, setRoomId] = useState('');
  
  // State for storing the last created room ID
  const [lastCreatedRoom, setLastCreatedRoom] = useState('');
  
  // State for clipboard copy status (shows "Copied!" briefly)
  const [isCopied, setIsCopied] = useState(false);

  /**
   * Handle Create Room Button Click
   * 
   * Calls the onCreateRoom callback and copies the new room ID
   * to the user's clipboard automatically.
   */
  const handleCreateRoom = async () => {
    try {
      // Call the parent component's createRoom function
      const newRoomId = await onCreateRoom();
      
      // Save the new room ID to state
   ...
   * Prevents default form behavior and calls onJoinRoom.
   */
  const handleJoinSubmit = (e) => {
    e.preventDefault(); // Prevent page reload
    
    // Only proceed if room ID is not empty
    if (roomId.trim()) {
      onJoinRoom(e); // Pass event to parent component
    }
  };
   ...
   * 
   * Helper function to copy any text to clipboard.
   * Used for copying room IDs.
   */
   ...
   * Main Render
   * 
   * Returns the JSX for the room component, including:
  ...
  return (
    <div className="max-w-md mx-auto">
      ...
      <div className="bg-white rounded-lg shadow-md p-6 mb-6">
        ...
        {lastCreatedRoom && (
          <div className="mb-4 p-4 bg-green-50 border border-green-200 rounded-lg">
            {/* Success header with copy button */}
        ...
        <div className="space-y-6">
          ...
          {/* Divider with "Or join existing" text */}
      ...
      {/* Recent Rooms History (shown after creating a room) */}
      {lastCreatedRoom && (
